...
async function calcCost(combination_set, criteria) {
    ...
    for (var i = 0; i < combination_set.length; i++) {
        ...
        for (var j = 1; j < combination_set[i]['path'].length; j++) {
            ...
            if(Object.keys(data).includes('error') && data['error']['code'] === '-98'){
                cost.msg = "출발지와 도착지가 700m 이내입니다.";
            }else{
...
/*  @POST("/map/location") fun requestMapLocation (@Body body: Map_Input : Singl
* e<ArrayList<Map_Output>>
...
module.exports.sendingFormat = sendingFormat;
